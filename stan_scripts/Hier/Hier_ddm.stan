...
transformed parameters{
  ...
  for(s in 1:S){
    ...
    delta[s] = gm[1]+((diag_pre_multiply(tau_u, L_u) * z_expo)'[s,1]);
    alpha[s] = exp(gm[2]+((diag_pre_multiply(tau_u, L_u) * z_expo)'[s,2]));
    beta[s] = inv_logit(gm[3]+((diag_pre_multiply(tau_u, L_u) * z_expo)'[s,3]));
    tau[s] = inv_logit(gm[4]+((diag_pre_multiply(tau_u, L_u) * z_expo)'[s,4]))* minRT[s];
  }
