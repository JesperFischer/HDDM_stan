...
pacman::p_load(tidyverse,RWiener, tidybayes, posterior, furrr,gganimate, cmdstanr,patchwork, gamlss,truncnorm,extraDistr,flextable)
...
alpha = rnorm(10,3,0.5)
delta = seq(-3,3,by = 1)
delta = rnorm(10,5,1)

beta = seq(0.2,0.8,by = 0.1)
beta = 0.5
beta = rnorm(10,0.8,0.1)

tau = seq(0.1,0.3,by = 0.1)
tau = 0.2
tau = rnorm(10,0.2,0.05)
...
params = params_rt %>% filter(trials == 50) %>% 
  arrange(subs) %>% 
  group_by(subs,trials) %>% 
  mutate(sessions = rep(1:(length(replicate)),each = 50))


mod = cmdstanr::cmdstan_model(here::here("report","DDM","Stan Models","DDM_ICC_4sessions.stan"))
...
mm = lmerTest::lmer(mean ~ real_alpha + (1|subs), data = params2)
...
hier_fitplot = data.frame(fit$summary(c("alpha","delta","tau","beta"))) %>% 
  mutate(subs1 = as.numeric(gsub(".*\\[(\\d+)\\]", "\\1", variable))) %>% 
  mutate(
    subs = case_when(
      grepl("1$", as.character(subs1)) ~ "1",
      grepl("2$", as.character(subs1)) ~ "2",
      grepl("3$", as.character(subs1)) ~ "3",
      grepl("4$", as.character(subs1)) ~ "4",
      grepl("5$", as.character(subs1)) ~ "5",
      grepl("6$", as.character(subs1)) ~ "6",
      grepl("7$", as.character(subs1)) ~ "7",
      grepl("8$", as.character(subs1)) ~ "8",
      grepl("9$", as.character(subs1)) ~ "9",
      TRUE ~ "10"  # If none of the conditions match, you can assign NA or any other default value
    )
...
hier_fitplot %>% 
  ...
  geom_hline(data = params2 %>% pivot_longer(cols = c("real_alpha","real_delta","real_beta","real_tau"), values_to = "real"),
             aes(yintercept = real))+
...
data.frame(fit$summary("delta")) %>% 
  ...
  mutate(
    subs = case_when(
      ...
      grepl("9$", as.character(subs1)) ~ "9",
      TRUE ~ "10"  # If none of the conditions match, you can assign NA or any other default value
    )) %>%   ggplot(aes(x = subs, y = mean, col = as.factor(subs)))+
  geom_pointrange(aes(y = mean, ymin = q5, max = q95),position=position_jitter(width = 2))+
  geom_hline(data = params2, aes(yintercept = real_delta))+facet_wrap(~subs)
