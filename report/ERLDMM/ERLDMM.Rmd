...
load(here::here("report","ERLDMM","parameter_recovery.RData"))
...
params %>% filter(variable == "sens_w") %>% 
  ggplot(aes(x = mean, fill = as.factor(real_sens_w)))+
  geom_histogram(col = "black", alpha = 0.5)+
  theme_classic()+
  geom_vline(aes(xintercept = real_sens_w))+
  facet_wrap(~trials)

params %>% filter(variable == "sens_c") %>% 
  ggplot(aes(x = mean, fill = as.factor(real_sens_c)))+
  geom_histogram(col = "black", alpha = 0.5)+
  theme_classic()+
  geom_vline(aes(xintercept = real_sens_c))+
  facet_wrap(~trials)
