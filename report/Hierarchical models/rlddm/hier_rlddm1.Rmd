...
#trials = round(runif(100,50,500),0)
#trials = 100
trials = sample(c(50,100,150,200),100, replace = T)

subjects = round(runif(100,5,50),0)
#subject = 10

mu_alpha = runif(100,0.5,4)
#mu_alpha = c(2,3)
sd_alpha = runif(100,0.5,2)
#sd_alpha = 0.5


mu_delta = runif(100,1,5)
#mu_delta = c(2,3)

sd_delta = runif(100,0.5,2)


#mu_beta = seq(0.4,0.6,by = 0.2)
#mu_beta = 0.5
mu_beta = runif(100,0.3,0.7)
sd_beta = runif(100,0.1,0.6)

mu_tau = runif(100,0.1,0.5)
sd_tau = runif(100,0.1,0.4)
...
parameters = data.frame(mu_alpha = mu_alpha,
                         sd_alpha = sd_alpha,
                         mu_delta = mu_delta,
                         sd_delta = sd_delta,
                         mu_beta = mu_beta,
...
# continous
```{r}
load(here::here("report","Hierarchical models","rlddm","parameter recovery","workspace","100 contin.RData"))
...
plot_list = list()
  for(variable1 in variables){
    ...
    if(variable1 == "mu_alpha"){
      renaming$mean = exp(renaming$mean)
      renaming$q95 = exp(renaming$q95)
      renaming$q5 = exp(renaming$q5)
      
    }
    if(variable1 == "mu_tau"){
      renaming$mean = brms::inv_logit_scaled(renaming$mean)
    }
    
    
    plot = renaming %>% filter(variable == variable1) %>% 
      mutate(real = get(paste0("real_",variable1))) %>% 
      ggplot(aes(x = mean, y = real, fill = subjects))+
      geom_pointrange(aes(xmin = q5,xmax = q95),alpha = 0.75, shape = 21)+
      #geom_point(alpha = 0.75, col = "black" position = "      
      theme_classic()+
      facet_grid(~trials)+
      {if(variable1 == "tau")coord_cartesian(xlim = c(0,.5))}+
      ggtitle(variable1)+
...
model_gam = gamlss::gamlss(mean ~ real,
                           sigma.formula = ~Trial_number*subjects,
                           data = m1)
summary(model_gam)


plot(ggeffects::ggpredict(model_gam, terms = c("Trial_number","subjects"), what = "sigma"))+
  geom_point()
