...
```{r}
agent2(parameters = data.frame(n_reversals = 5,
...
trials_per_reversal = seq(20,40, length.out = 3)
...
seed = sample(round(rnorm(1000,1000,100),0),5,replace = F)
...
```{r}
agent3(parameters = data.frame(seed = 1113,
                        trials_per_reversal = 20,
                        theta = 5,
                        omega = -1,
                        kappa = 1,
...
parameters = data.frame(seed = 1113,
                        trials_per_reversal = 30,
                        theta = 0.5,
                        omega = -1,
                        kappa = 1,
                        beta = 3)
...
results <- future_map(data_list, ~agent3(.x), .progress = TRUE, .options = furrr_options(seed = TRUE))
...
params %>% group_by(t_p_r,omega,theta,beta, seed) %>% summarize(mean = mean(pcorrect)) %>% 
  mutate(mean = round(mean,2)) %>% 
...
params %>% group_by(t_p_r,omega,theta,beta, kappa) %>% summarize(mean = mean(pcorrect)) %>% 
  mutate(mean = round(mean,2)) %>% 
  ggplot(aes(y = omega, x = theta))+
  geom_raster(aes(fill = mean))+
  facet_grid(t_p_r~kappa, labeller = label_both)+
  scale_fill_continuous(type = "viridis",)+
  theme_classic()+
  geom_point(data = optimals, size = 1)+
  geom_point(data = optimal, size = 2, col = "red")+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
resp %>% ggplot(aes(x = trial, y = percept, col = as.factor(stim)))+geom_point()+geom_smooth(method = "lm")

resp %>% ggplot(aes(x = expectation, y = percept, col = as.factor(stim)))+geom_point()+geom_smooth(method = "lm")
