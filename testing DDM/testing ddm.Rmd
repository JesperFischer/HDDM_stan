...
load(here::here("testing DDM","results_ddm_data.RData"))
...
params%>% slice(seq(1,nrow(params), by = 2)) %>% filter(amount < -2) %>% group_by(trials,winning_model) %>% summarize(n())
...
params %>% filter(winning_model == "ddm") %>% ggplot(aes(x = as.factor(real_alpha), y = amount, col = as.factor(trials)))+
...
We start with the models that caused errors if any.
```{r}
load(here::here("testing DDM","results ln.RData"))
...
#params %>% filter(amount < -2) %>% group_by(winning_model) %>% summarize(n())
params%>% slice(seq(1,nrow(params), by = 2)) %>% filter(amount < -2) %>% group_by(trials,winning_model) %>% summarize(n())



pp = params %>% slice(seq(1,nrow(params), by = 2)) %>% filter(winning_model == "lognormal")

```


```{r, fig.height=6, fig.width=8}
params %>% filter(winning_model == "lognormal")%>% ggplot(aes(x = as.factor(real_intercept), y = amount, col = as.factor(trials)))+
  geom_boxplot()+theme_classic()+geom_abline(intercept = -2, slope = 0)+
...
load(here::here("testing DDM","results ln_data_nobeta.RData"))
error_indices <- which(results_ln == "Error")
...
load(here::here("testing DDM","ddm data on nobeta lognormal.RData"))
...
```{r, fig.height=6, fig.width=8}
params %>% filter(winning_model == "ddm")%>% slice(seq(1,nrow(params), by = 2)) %>% filter(amount < -2) %>% ggplot(aes(x = as.factor(real_alpha), y = amount, col = as.factor(trials)))+
  geom_boxplot()+theme_classic()+geom_abline(intercept = -2, slope = 0)+
  facet_grid(real_delta~real_beta, labeller = label_both, scales = "free")
